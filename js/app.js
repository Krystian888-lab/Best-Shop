...
function Calculator( form, sumary ) {
 this.prices = {
  products: 0.5,
  orders: 0.25,
  package: {
...
// Summary elements
 this.summary = {
  list: sumary.querySelector( "ul" ),
  items: sumary.querySelector( "ul" ).children,
  total: {
   container: sumary.querySelector( "#total-price" ),
   price: sumary.querySelector( ".total__price" )
  }
 };
...
Calculator.prototype.inputEvent = function ( el ) {
 ...
 this.updateSummary( id, value + " * $" + singlePrice, totalPrice, function ( item, calc, total ) {
  if ( value < 0 ) {
   calc.innerHTML = null;
   total.innerText = "Value should be greater then 0";
  }
...
Calculator.prototype.updateTotal = function () {
 ...
 if ( show ) {
  const productSum = this.form.products.value < 0
  ? 0 
